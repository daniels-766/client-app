...
# =============== Konfigurasi ===============
LINUX_SERVER = "http://192.168.88.90:7000"  # ganti sesuai IP server Linux
CLIENT_PORT = 6000                          # port Flask di Windows Client
# ==========================================

flask_app = Flask(__name__)
client_ui = None

# ---------- Util: ambil IP lokal yang bisa diakses ----------
def get_local_ip():
    try:
        # Cara yang lebih akurat untuk mendapat IP LAN
...
# ---------- Endpoint menerima data/progress dari Server Linux ----------
...
class ClientUI:
    ...
    def show_data(self, data):
        user = data.get("user", {})
        self.user_info.config(text=f"ID: {user.get('id_system')}\nUsername: {user.get('username')}\nPhone: {user.get('phone')}")
        ...
        # tampilkan nomor yang sedang dipanggil (progress)
        progress = data.get("progress")
        if progress:
            phase = progress.get('phase', '-')
            number = progress.get('number', '-')
            answered = progress.get('answered', False)
