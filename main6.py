...
def receive_info():
    data = request.json
    # Forward ke Linux server kalau ini datang dari dashboard dan belum diforward
    try:
        origin = data.get("origin")
    except Exception:
        origin = None

    if origin == "dashboard" and not data.get("server_forwarded"):
        try:
            fwd = dict(data)
            fwd["server_forwarded"] = True  # supaya Linux broadcast balik tidak di-forward lagi
            requests.post(f"{LINUX_SERVER}/push-data", json=fwd, timeout=8)
