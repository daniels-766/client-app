...
# ======= State global =======
connected_clients = set()    # contoh: "http://192.168.88.10:6000"
call_queue = Queue()
...
class _CallCb(pj.CallCallback):
    """Callback untuk panggilan outgoing; PJSIP akan mengisi self.call."""
...
class SipManager:
    ...
    def dial_then_transfer_to_agent(self, target_number: str, ring_timeout_sec: int, agent_user: str | None):
        """
        1) Dial target_number
        2) Jika answered dan agent_user diberikan -> REFER ke agent_user@domain:port
        3) Hangup leg dialer
        """
        if not self.acc:
        ...
        if answered and agent_user:
            try:
                ...
                call.xfer(refer_to)  # unattended transfer
...
def call_flow_worker():
    while True:
        ...
        username = item.get("_sip_user")  # agent SIP (juga login di MicroSIP Windows)
        ...
        for label, number in numbers:
            ...
            # Jika NASABAH terjawab & ditransfer, selesai item
            if label == "NASABAH" and result["answered"]:
                break
            # Jika EC terjawab (tanpa transfer), anggap selesai
