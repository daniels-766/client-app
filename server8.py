...
SIP_DIAL_SUFFIX = f"@{SIP_HOSTPORT}"   # jadinya: sip:<number>@ld.infin8link.com:6070
# ===========================================================
...
class _CallCb(pj.CallCallback):
    """Callback panggilan outgoing; akan diberi call oleh PJSIP."""
...
class SipManager:
    ...
    def _init_lib(self):
        ...
        self.lib.start()
        # ====== MATIKAN AUDIO DEVICE DI LINUX (HEADLESS) ======
    ...
    def ensure_account(self, username: str, password: str):
        with self.lock:
            ...
            # Paksa UDP? uncomment jika perlu:
    ...
    def dial_then_transfer_to_agent(self, target_number: str, ring_timeout_sec: int, agent_user: str | None):
        ...
        3) Hangup leg dialer (media akan berjalan antara agent<->nasabah via SIP server)
        """
        if not self.acc:
        ...
        if answered and agent_user:
            try:
                ...
                call.xfer(refer_to)  # unattended transfer (REFER)
