...
LINUX_SERVER = "http://192.168.88.90:7000"
CLIENT_PORT = 6000
...
class ClientUI:
    ...
    def show_data(self, data):
        ...
        for idx, d in enumerate(data.get("data", [])):
            ...
            self.tree.insert("", "end",
                             values=(d['nama_nasabah'], d['phone'], d['ec_name_1'], d['ec_phone_1'],
                                     d['ec_name_2'], d['ec_phone_2'], d['total_tagihan']),
                             tags=(tag,))
    ...
    def send_command(self, action):
        try:
            ...
            msg = f"[OK] {action.upper()} -> {res.json()}\n" if res.status_code==200 else f"[ERROR] {action.upper()} gagal ({res.status_code})\n"
    ...
    def register_to_server(self):
        try:
            ...
            local_ip = "127.0.0.1"
        try:
            res = requests.post(f"{LINUX_SERVER}/register-client", json={"ip": local_ip}, timeout=5)
